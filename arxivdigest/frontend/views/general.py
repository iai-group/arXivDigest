...
from arxivdigest.frontend.database.articles import (
    article_is_recommended_for_user,
    get_article_feedback,
)
from arxivdigest.frontend.forms.feedback_form import (
    ArticleFeedbackForm,
    FeedbackForm,
)
...
from arxivdigest.frontend.utils import (
    create_gzip_response,
    encode_auth_token,
    requiresLogin,
...
def login():
    ...
    return make_auth_token_response(
        user, data.get("email"), url_for("articles.index")
    )
...
def signup():
    ...
    return make_auth_token_response(
        id, user.email, url_for("general.confirm_email_page")
    )
...
def signupPage():
    ...
    if g.loggedIn:
        return redirect(url_for("articles.index"))
    return render_template(
        "signup.html", signup=True, categoryList=db.getCategoryNames()
    )
...
def modify():
    ...
    try:
        ...
        flash(e.message, "danger")
        return render_template(
            "modify.html", user=user_dict, categoryList=db.getCategoryNames()
        )
    if db.userExist(user.email):
        flash("Email already used by another account.", "danger")
        return render_template(
...
def registerSystem():
    ...
    if len(form["name"]) > CONSTANTS.max_system_name_length:
        flash(
            "System name must be under {} characters.".format(
                CONSTANTS.max_system_name_length
            ),
            "danger",
        )
...
def feedback():
    ...
    if form.validate_on_submit():
        err = db.insertFeedback(
            g.user, None, form.feedback_type.data, form.feedback_text.data, {}
        )
        if err:
            flash(err, "danger")
...
def article_feedback(article_id):
    ...
    if not article_is_recommended_for_user(article_id):
        flash(
            "You can only leave feedback on articles recommended to you.",
            "danger",
        )
        return redirect("/")
    ...
    if form.validate_on_submit():
        ...
        if err:
            flash(err, "danger")
            return render_template(
                "feedback.html", form=form, article_id=article_id
            )

        flash("Successfully sent feedback.", "success")
        return redirect("/")
    return render_template(
        "feedback.html", form=form, article=get_article_feedback(article_id)
    )
...
def confirm_email_page():
    ...
    if general_db.is_activated(g.user):
        ...
        return make_auth_token_response(
            g.user, g.email, url_for("articles.index")
        )
...
def send_email():
    ...
    return make_auth_token_response(
        g.user, email, url_for("general.confirm_email_page")
    )
...
def update_semantic_scholar():
    ...
    if data["s2Id"] != "none":
        ...
        db.update_semantic_scholar(
            f'https://semanticscholar.org/author/{data["s2Id"]}', g.user
        )
